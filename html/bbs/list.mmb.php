...
$stx = trim($stx);
if ($sca || $stx || $hash || $log) {
	// 검색구문 호출 기능 커스텀한 것으로 재호출
	$sql_search = get_sql_search_mmb($sca, $sfl, $stx, $hash, $sop, $log, $single);
...
$hash_pattern = "/\\#([0-9a-zA-Z가-힣_]*)/";
$real_str = preg_replace($hash_pattern, '%23$1', $_REQUEST['hash']);

$qstr .="&amp;hash=".$real_str."&amp;log=".$_REQUEST['log']."&amp;single=".$single;
