...
for ($index=0; $index<count($comment); $index++) {
	...
	if(!$log_comment['wr_noname']) { 
		...
		if($log_comment['mb_id']) {
			$log_comment['name'] = "<a href='http://twitter.com/".$log_comment['wr_name']."' target='_blank'>"."@".$log_comment['wr_name']."</a>";
		} else { 
...
<div class="item-comment" id="c_<?php echo $comment_id ?>">
	<div class="co-content">
		...
		<? if(!$log_comment['wr_noname']) { ?>
		<div class="item_wr">
			<div class="ch_thumb_round" style="background-image:url('<?=$ch['ch_thumb']?>')">
			</div>
		...
		<div>
			<div class="original_comment_area system_div">
				<? echo html_entity_decode($log_comment['content']); ?>
			</div>
			<span class="date">
