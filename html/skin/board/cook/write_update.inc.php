...
if($w != 'cu') {
	...
	if($action == 'H') { 
		...
		if(!$re['it_id']) {
			...
			// 팀원 수 구하기
			$team =sql_fetch("select count(*) as cnt from {$g5['character_table']} where ch_side = '{$character['ch_side']}' and ch_state = '승인'");
			$item = get_item($re['it_id']);
			$item_qty = $team ?  $team['cnt'] : 1;
			insert_inventory($character['ch_id'], $item['it_id'], $item, $item_qty);
