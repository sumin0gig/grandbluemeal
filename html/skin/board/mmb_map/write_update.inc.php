...
if($w != 'cu') {
	...
	******************************************************/	
			...
			if($me['me_id']) {
				...
				if($me['me_type'] == '') { 
					...
					if(!isset($temp_check['wr_mon_state'])) { 
						...
						sql_query(" ALTER TABLE `{$write_table}` ADD `wr_mon_rewards` varchar(255) NOT NULL default '' AFTER `wr_10` ");
						sql_query(" ALTER TABLE `{$write_table}` ADD `wr_mon_rw_per` varchar(255) NOT NULL default '' AFTER `wr_10` ");
					}
					unset($temp_check);

					// 몬스터 상태 추가
					$customer_sql .= " , wr_mon_state='S', wr_mon_hp='{$me['me_mon_hp']}', wr_mon_now_hp='{$me['me_mon_hp']}', wr_mon_attack='{$me['me_mon_attack']}', wr_mon_rewards='{$me['me_mon_rewards']}', wr_mon_rw_per='{$me['me_mon_rw_per']}' ";
				}
			}
	...
	if($action == 'MAP_MON' || $action == 'MAP_MON_MAGIC' ) { 
		...
		*********************************/
		if ($action == "MAP_MON") {
			$state_num = 2;
			$state = "cl_str";
		}else if($action == "MAP_MON_MAGIC"){
			$state_num = 3;
			$state = "cl_mp";
		}
		$state_result = sql_fetch("select * from {$g5['status_table']} where ch_id = '{$character['ch_id']}' and st_id = '{$state_num}'");
		$state_status =  $state_result['sc_max']-$state_result['sc_value'];
		$dice_attack1 = rand( max($state_status-3 , 0) , $state_status );		// 스탯 주사위

		$class_result = sql_fetch("select * from {$g5['class_table']} where cl_id = '{$character['ch_class']}'");
		$dice_attack2 = rand( 0, $class_result[$state] );		// 종족 스탯 주사위
		...
		if($mon_hp > 0) { 
			...
			/****************************************************************************************
				유저 HP 차감
			****************************************************************************************/
			$hp_result = sql_fetch("select * from {$g5['status_table']} where ch_id = '{$character['ch_id']}' and st_id = '1'");
			$minus_hp = min( $hp_result['sc_max'], $hp_result['sc_value']+$mon_attack );
			sql_fetch("update {$g5['status_table']} set sc_value = '{$minus_hp}' where ch_id = '{$character['ch_id']}' and st_id = '1'");
			
			/****************************************************************************************
				유저 HP 차감
			****************************************************************************************/

			$mon_hp = $mon_hp - $attack;
			$get_item_list = "";
			if($mon_hp < 0) {

				// 막타일 경우 아이템 획득
				if(!empty($origin['wr_mon_rewards']) && !empty($origin['wr_mon_rw_per'])){
					$wr_mon_rewards = explode("|",$origin['wr_mon_rewards']);
					$wr_mon_rw_per = explode("|",$origin['wr_mon_rw_per']);

						// 같이 공격한 인원 구하기
						$wr_parent = $origin['wr_parent'];
						$attackers = sql_query("select ch_id from {$write_table} where wr_parent = '{$wr_parent}' and wr_log is not null group by ch_id");
						$get_item = false;
						if ( sql_num_rows($attackers) > 1 ){
							// for ($t=0; $t < sql_fetch_array($attackers); $t++) {
							while( $row = sql_fetch_array($attackers) ){
								$attacker_id = $row['ch_id'];
								$attacker = sql_fetch("select * from {$g5['character_table']} where ch_id = '{$attacker_id}'");
								$get_item_list .= $attacker['ch_name']."은(는) ".$t;
								$rand_dice = rand(0,100);

								for ($i=0; $i < count($wr_mon_rewards); $i++) { 
									$rw_per = explode("~",$wr_mon_rw_per[$i]);
									// 아이템 획득을 위한 확률 계산
									$min_probability = intval($rw_per[0]);
									$max_probability = intval($rw_per[1]);

									// 확률에 따라 아이템 획득
									if ($min_probability <= $rand_dice && $rand_dice <= $max_probability) {
										$get_item = true;
										$item = get_item($wr_mon_rewards[$i]);
										$get_item_list .= " 「 ".$item['it_name']." 」 ";
										insert_inventory($attacker['ch_id'], $item['it_id'], $item);
									}
								}

								$get_item_list .= $get_item ? "을(를) 획득했다. " : "아무것도 획득하지 못했다. ";
							}
						}else{
							$get_item_list .= $character['ch_name']."은(는) ";
							$rand_dice = rand(0,100);
			...
			$ch_hp = $hp_result['sc_max'] - $minus_hp;
			$log = $action."||{$mon_state}||".$mon_attack."||".$attack."||".$ch_hp."||".$dice_attack1."+".$dice_attack2."+".$item_attack."||".$get_item_list;
			$customer_sql .= " , wr_log = '{$log}' ";
		}
	}
