...
if(!$in['in_id']) { 
	...
	if($inven_function == "식사") {
		$msg = eat_food(intval($ch['ch_id']), intval($in['in_id']), intval($in['it_exp']), intval($in['st_id']), (intval($in['it_value']) * -1), $in['it_name']);
		echo location_url($msg );
	}
