	...
	<div class="tbl_frm01 tbl_wrap">
		<table>
			...
			<tbody>
				...
				<tr>
					<th scope="row">몬스터 처치 보상 설정</th>
					<td colspan="2">
						<?php echo help("※ 몬스터를 처치했을 때 얻을 보상, 아이템 설정에서 처치 보상에 체크 되어있을때만 검색 가능") ?>
						<input type="text" name="me_mon_reward" id="me_mon_reward" onkeyup="get_ajax_item(this, 'reward_item_list', 'me_mon_reward', '처치');" placeholder="검색창"/>
						<div id="reward_item_list" class="ajax-list-box"><div class="list"></div></div>

						<?php echo help("※ 검색창에서 아이템 ID를 검색하여 나온 값을 |로 구분하여 작성합니다. ex: 2|3") ?>
						<input type="text" name="me_mon_rewards" value="<?=$me['me_mon_rewards']?>" id="me_mon_rewards" size="100" placeholder="아이템 IDS"/>
						<? $rewards = !empty($me['me_mon_rewards']) ? explode("|", $me['me_mon_rewards']) : array(); ?>
						<div> <b>현재 등록된 아이템: </b>
						<? for($r=0; $r < count($rewards); $r++) { ?>
							(<?= $rewards[$r] ?>)<?= get_item_name($rewards[$r]) ?>
						<? } ?>
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">몬스터 처치 보상 확률 설정</th>
					<td colspan="2">
						<?php echo help("※ 몬스터를 처치했을 시 보상을 |로 구분하여 작성합니다. ex: 0~100|3~5") ?>
						<input type="text" name="me_mon_rw_per" value="<?php echo $me['me_mon_rw_per']; ?>" id="me_mon_rw_per" size="100">
					</td>
				</tr>
			</tbody>
		</table>
			...
			<tbody>
				<tr>
					...
					<td colspan="2">
						...
						<input type="text" name="it_name" value="<?=get_item_name($me['me_get_item'])?>" id="it_name" onkeyup="get_ajax_item(this, 'item_list', 'me_get_item', '탐색');" />
