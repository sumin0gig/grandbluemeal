...
if($use_recepi) { $sql_search .= " and it_use_recepi = '{$use_recepi}' "; }
if($use_search) { $sql_search .= " and it_use_search = '{$use_search}' "; }
if($use_reward) { $sql_search .= " and it_use_reward = '{$use_reward}' "; }

if (!$sst) {
	$sst  = $_GET['sst'] ?? "it_name";
	if ($_GET['sst'] == "") {
		$sst = "it_name";
	}
	$sod = $_GET['sod'] ?? "asc";
...
$listall = '<a href="'.$_SERVER['PHP_SELF'].'" class="ov_listall">전체목록(가나다순)</a> <a href="'.$_SERVER['PHP_SELF'].'?sst=it_id&sod=asc" class="ov_listall">전체목록(id순)</a>';
...
<select name="use_recepi" id="use_recepi">
	<option value="">레시피 재료</option>
	<option value="1" <?=$use_recepi == "1" ? "selected" : ""?>>레시피 사용</option>
	<option value="0" <?=$use_recepi == "0" ? "selected" : ""?>>레시피 미사용</option>
</select>
<select name="use_search" id="use_search">
	<option value="">탐색 보상</option>
	<option value="1" <?=$use_search == "1" ? "selected" : ""?>>탐색 사용</option>
	<option value="0" <?=$use_search == "0" ? "selected" : ""?>>탐색 미사용</option>
</select>
<select name="use_reward" id="use_reward">
	<option value="">처치 보상</option>
	<option value="1" <?=$use_reward == "1" ? "selected" : ""?>>처치 사용</option>
	<option value="0" <?=$use_reward == "0" ? "selected" : ""?>>처치 미사용</option>
</select>
	...
	<table>
		...
		<thead>
			<tr>
				...
				<th scope="col">인벤</th>
				<th scope="col">자비</th>
				<th scope="col">레시피 재료</th>
				<th scope="col">탐색 보상</th>
				<th scope="col">처치 보상</th>
		...
		<tbody>
			...
			<tr class="<?php echo $bg; ?>">
				...
				<td>
					<input type="checkbox" name="dic_show[<?php echo $i ?>]" value="1" id="dic_show_<?php echo $i ?>" <?php echo $item['dic_show']?"checked":"" ?>>
				</td>
				<td>
				...
				<td>
					<input type="checkbox" name="it_use_search[<?php echo $i ?>]" value="1" id="it_use_search_<?php echo $i ?>" <?php echo $item['it_use_search']?"checked":"" ?>>
				</td>
				<td>
					<input type="checkbox" name="it_use_reward[<?php echo $i ?>]" value="1" id="it_use_reward_<?php echo $i ?>" <?php echo $item['it_use_reward']?"checked":"" ?>>
				</td>
				<td>
