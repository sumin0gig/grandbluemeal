...
function editor_html($id, $content, $is_dhtml_editor=true)
{
    ...
    $html .= "\n<textarea id=\"$id\" name=\"$id\" class=\"$smarteditor_class\" maxlength=\"65536\" style=\"width:100%;height:auto\">$content</textarea>";
...
{
    if ($is_dhtml_editor) {
        return "if (!{$id}_editor_data || jQuery.inArray({$id}_editor_data.toLowerCase(), ['&nbsp;','<p>&nbsp;</p>','<p><br></p>','<p></p>','<br>']) != -1) { alert(\"내용을 입력해 주십시오.4\"); oEditors.getById['{$id}'].exec('FOCUS'); return false; }\n";
    } else {
