...
// 상점 카테고리 정보
$shop_brand = explode("||", $config['cf_shop_brand']);
...
$sql_search = " where sh_use = '1' and ca_name = '{$cate}' and br_name = '{$br}'";
...
$write_pages = get_paging(5, $page, $total_page, './index.php?br='.$br.'cate='.$cate.$qstr.'&amp;page=');
...
<script>
function view_shop_item(shop_idx,br) {
	var h_link = "./_ajax.shop_item.php?sh_id=" + shop_idx + "&br=" + br;
	$.ajax({
		async: true
		, url: h_link
		, beforeSend: function() {}
...
function fn_buy_item(shop_idx,item_qty,shop_brand){
	var h_link = "./_ajax.shop_update.php?sh_id=" + shop_idx + "&item_qty=" + item_qty;
	$.ajax({
		async: true
		, url: h_link
		, beforeSend: function() {}
