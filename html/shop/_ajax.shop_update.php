...
if($character['ch_id'] && $character['ch_state'] == '승인') { 
	...
	if($item['sh_id']) {
		...
		$qty = empty($item_qty) ? 1 : $item_qty;

		if($item['sh_money']) {
			// 구매가격이 존재 시
			if($member['mb_point'] < $item['sh_money'] * $qty ) {
				// 소지금 부족
				$notice .= "▶ ".$config['cf_money']."이 부족했다...<br />";
				$is_able_buy = false;

			} else if($item['sh_use_money']) {
				// 소지금 소모
				$use_point += $item['sh_money'] * $qty;
			}
		}

		if($item['sh_exp']) {
			// 구매 경험치 존재 시
			if($character['ch_exp'] < $item['sh_exp'] * $qty) {
				// 경험치 부족
				switch ($variable) {
					case '정육점':
						$msg .= "애송이";
				...
				$notice .= "▶ 아직 충분한 ".$config['cf_exp_name']."이 쌓이지 않았다...<br />";
				$is_able_buy = false;

			} else if($item['sh_use_exp']) {
				// 경험치 소모
				$use_exp_point += $item['sh_exp'] * $qty;
			}
		}
		...
		if($item['sh_has_item']) {
			...
			if($in['cnt'] < $item['sh_has_item_count'] * $qty) {
				// 필요 아이템 미소유
				$has_item_name = get_item_name($item['sh_has_item']);
				$notice .= "▶ ".$has_item_name.j($has_item_name, '이')." ".$item['sh_has_item_count']."개 있어야 살 수 있을것 같다...<br />";
				$is_able_buy = false;
			} else if($item['sh_use_has_item'] * $qty) {
				// 아이템 소모
		...
		if($item['sh_has_title']) {
			...
			if(!$ti['hi_id']) {
				...
				$notice .= "▶ ".$has_title['ti_title'].j($has_title['ti_title'], '이')." 있어야 살 수 있을것 같다...<br />";
				$is_able_buy = false;
		...
		if($item['sh_limit']) {
			...
			if($order['cnt'] > $qty){
				$notice .= "▶ 너무 많이 주문했다.<br />".$order['cnt']."개 까지만 주문 할 수 있다...<br />";
				$is_able_buy = false;
			}else if($order['cnt'] >= $item['sh_limit']) {
				$notice .= "▶ 너무 많이 구매했다.<br />더이상 구매할 수 없다...<br />";
				$is_able_buy = false;
			}
		}
		...
		if($item['sh_qty']) {
			...
			if($order['cnt'] > $qty){
				$notice .= "▶ 너무 많이 주문했다...<br />";
				$is_able_buy = false;
			}else if($order['cnt'] >= $item['sh_qty']) {
				$notice .= "▶ 재고가 모두 소진된 것 같다.<br />더이상 구매할 수 없다...<br />";
				$is_able_buy = false;
			}
		}
		...
		if($item['sh_class']) {
			...
			if(!strstr($item['sh_class'], '||'.$character['ch_class'].'||')) {
				$notice .= "▶ ".$config['cf_class_title']."외에는 팔고 싶지 않은 것 같다...<br />";
				$is_able_buy = false;
			}
		}
		if($item['sh_side']) {
			...
			if(!strstr($item['sh_side'], '||'.$character['ch_side'].'||')) {
				$notice .= "▶ ".$config['cf_side_title']."외에는 팔고 싶지 않은 것 같다...<br />";
				$is_able_buy = false;
			}
		}
		...
		if($is_able_buy) {
			...
			$notice = "▶ 「 ".$item['it_name']." 」 을(를) ".$qty."개 구입했다.";
		}
	}
} else {
	$notice = "<p style='color:red; padding-top:5px;'>부정사용 적발 시, 접근 차단이 될 수 있습니다.</p>";
}
